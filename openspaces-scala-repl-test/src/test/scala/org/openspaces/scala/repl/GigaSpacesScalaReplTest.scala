...
class GigaSpacesScalaReplTest {
  ...
  @Test
  def implicitAdminTest() = {
    val output = replOutputFor("admin")
    Assert.assertTrue(output.startsWith("res0: org.openspaces.admin.Admin = org.openspaces.admin.internal.admin.DefaultAdmin@"))
  }
      ...
      Console.withOut(ostream) {
        ...
        val repl = new GigaSpacesScalaReplLoop(Some(input), output) {
          override protected def getBaseDirectory =
            Properties.propOrElse("user.dir", ".") + "/../openspaces-scala/tools/scala/conf"
        }
